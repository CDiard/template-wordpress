...
class Walker_Nav_Flat extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0): void
    {
        ...
        if (is_search()) {
            ...
            if ((int)$data_object->object_id === (int)$blog_page_id) {
                $is_active = false;
            }
        }
        ...
        $output .= $indent . '<a href="' . esc_url($data_object->url) . '" type="link" class="nav-item nav-link link-body-emphasis ' . esc_attr($class_names) . '" title="' . esc_html($title) . '">';
